    ...
    <div class="auth-page">
      <section class="auth-card">
        ...
        <header class="auth-card__header">
          <h2 class="auth-card__title">Recuperar acceso</h2>
          <p class="auth-card__sub">Solicita codigo para recuperar acceso a tu cuenta.</p>
        </header>
        ...
        <form [formGroup]="form" (ngSubmit)="submit()">
          ...
          <ds-input
            label="Tenant ID"
            inputId="recover-tenant"
            type="text"
            formControlName="tenantId"
            [error]="err('tenantId')"
          />

          <ds-button type="submit">Enviar codigo</ds-button>
          <div class="auth-card__links">
            <a routerLink="/verify-code">Ya tengo codigo</a>
          </div>
        </form>
...
export class AuthRecoverPage {
  ...
  async submit() {
    ...
    try {
      await this.auth.recover(dto.email, dto.tenantId);
      this.message = 'Codigo enviado (si backend esta habilitado).';
      sessionStorage.setItem('recover_email', dto.email);
      sessionStorage.setItem('recover_tenant', dto.tenantId);
    } catch (error) {
      this.message = error instanceof Error ? error.message : 'Error en recover';
    }
  }

  protected err(name: 'email' | 'tenantId'): string | null {
